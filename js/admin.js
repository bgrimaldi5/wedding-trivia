...
async function resetGame() {
  if (!confirm('⚠️ RESET the entire game?\n\nThis will remove ALL players, answers, and scores.\nQuestions will be kept.\n\nThis cannot be undone!')) return;

  showToast('⏳ Resetting...');

  // Delete all answers (filter by a condition that matches every row)
  ...
  if (answersErr) {
    console.error('Error deleting answers:', answersErr);
  }
  ...
  const { error: playersErr } = await db
    .from('players')
    .delete()
    .gte('joined_at', '1970-01-01');

  if (playersErr) {
    console.error('Error deleting players:', playersErr);
    showToast('❌ Reset failed — check Supabase RLS policies');
    return;
  }

  // Generate a new reset token so all active players get booted to the join page
  ...
  showToast('🔄 Game reset — all players and scores cleared');
}
