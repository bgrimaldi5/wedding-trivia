...
async function importQuestions() {
  ...
  // Step 1: Fetch and delete existing questions one by one
  ...
  try {
    ...
    if (existingQuestions && existingQuestions.length > 0) {
      // Delete answers linked to each question first
      for (const q of existingQuestions) {
        await db.from('answers').delete().eq('question_id', q.id);
      }
      // Then delete each question
      for (const q of existingQuestions) {
        await db.from('questions').delete().eq('id', q.id);
      }
      log(`✓ Cleared ${existingQuestions.length} existing question(s).`, 'ok');
    } else {
      log('✓ No existing questions to clear.', 'ok');
    }
  } catch (err) {
    log(`⚠️  Could not clear existing data: ${err.message} — continuing anyway.`, 'info');
  }

  log(`Importing ${parsedQuestions.length} questions...`, 'info');
