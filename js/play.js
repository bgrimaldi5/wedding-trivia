...
async function renderScreen(gameState) {
  ...
  if (status === 'lobby') {
    // Check if the reset token has changed — if so, this player was wiped
    // and needs to re-enter their name
    const savedToken = getResetToken();
    if (savedToken !== gameState.reset_token) {
      clearPlayerSession();
      window.location.href = 'index.html';
      return;
    }
...
async function renderLeaderboard() {
  ...
  let rankMessage = '';
  if (myRank === 1) {
    rankMessage = "🏆 First Place! Ben has reviewed the data and confirmed you are, statistically, the best person here.";
  } else if (myRank === 2) {
    rankMessage = "🥈 Second Place! Josie would like you to know she is proud of you. Ben would like you to know you lost.";
  } else if (myRank === 3) {
    rankMessage = "🥉 Third Place! You got to lay on the beach, eat some good food, and you finished on the podium. Honestly a great day.";
  } else if (myRank === 4) {
    rankMessage = "🫡 You came in fourth. You are proof that sometimes trying your absolute hardest still isn't quite enough. Inspiring, really.";
  } else if (myRank === total) {
    rankMessage = "💀 Absolute bottom of the barrel. Genuinely impressive how little you know about the people whose wedding you're attending.";
  } else if (myRank >= Math.floor(total * 0.75)) {
    rankMessage = "😬 You're in the bottom of the pack. Have you met Ben and Josie or did you just follow someone in off the street?";
  } else if (myRank <= Math.ceil(total / 2)) {
    rankMessage = "👏 Not a bad showing. Not first, not last, firmly in the 'I actually know the couple' category. Well done.";
  } else {
    rankMessage = "🤷 Somewhere in the middle. You showed up, you played, you did not embarrass yourself. You better make up for it on the dance floor.";
  }
